...
def main(H):
    ...
    # Simple Transformer
    n_emb = 512
    transformer = GPT(H.codebook_size+1, 512, H.latent_shape, ae.quantize.embedding, n_layer=16, n_head=8, n_embd=n_emb).cuda()
    optim = torch.optim.Adam(transformer.parameters(), lr=1e-4)

    start_step = 0
    if H.load_step > 0:
        transformer = load_model(transformer, 'transformer', H.load_step, log_dir)
        if H.load_optim:
            optim = load_model(optim, 'transformer_optim', H.load_step, log_dir)
